...
class CardSort extends React.Component<any, any> {
  ...
  updateDragAndDrop(dragId: any, dropId?: any) {
    ...
    if (!dropId) {
      ...
      let beginDragId = -1;
      [moveDragItem, dragCardList] = this.removeDragItemById(
        dragId,
        dragCardList
      );

      if (!moveDragItem) {
        [moveDragItem, dropCardList, beginDragId] = this.removeDropItemById(
          dragId,
          dropCardList
        );
      }

      dropCardList = dropCardList.map(
        (item: { id: any; innerDrag: any }, _index: any, arr: any) => {
          if (item.id === dropId) {
            if (item.innerDrag) {
  ...
  dragItemBack(dragId: number) {
    ...
    [moveDragItem, dropCardList] = this.removeDropItemById(
      dragId,
      dropCardList
    );
  ...
  removeDropItemById(itemId: any, dropCardList: any) {
    let moveDragItem: any = null;
    let dropId = -1;
    let newDropCardList = dropCardList.map(
      (item: { id: any; innerDrag: any }, index: number) => {
